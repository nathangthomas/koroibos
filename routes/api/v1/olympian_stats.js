...
async function averageMaleWeight() {
  let totalWeight = 0
  var men = await database('olympics_data').where('sex', '=', 'M').whereNotNull('weight').select('name').groupBy('name')
...
async function averageFemaleWeight() {
  let totalWeight = 0
  var women = await database('olympics_data').where('sex', '=', 'F').whereNotNull('weight').select('name').groupBy('name')
